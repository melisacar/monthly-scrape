...
def main_check():
    ...
    if newest_month and newest_month > latest_month:  
        for href in hrefs:
            print(f"Yeni dosya işleniyor: {href}")
            ...
            if excel_content:
                ...
                with pd.ExcelWriter("DHMI_all.xlsx", engine='openpyxl', mode='a') as writer:
                    df.to_excel(writer, header=False, index=False)  # Add.
            else:
                print(f"{href} indirme hatasından dolayı atlandı.")
